...
export default function SimpleHomepage({
  ...
  let breaking = null;
  if (nonDupObjects) {
    const breakingIndex = nonDupObjects.findIndex(
      (o) => o.object.data.sys.type === "breakingnews"
    );
    if (breakingIndex >= 0) {
  ...
  let opening = null;
  const articleCount = nonDupObjects.filter(
    (o) => o.object.data.sys.baseType === "article"
  ).length;
  if (articleCount <= 3) {
    opening = (
