...
export default function RenderContentElement({
  jsonElement,
  excludeElements,
  renderMode,
  neonData,
      ...
      switch (jsonElement.name) {
          ...
          render = (
            <React.Fragment>
              {jsonElement.elements
                ? jsonElement.elements.map((subel) => subel.text)
                : null}
            </React.Fragment>
          );
        ...
        case "table":
          if (
            ...
            jsonElement.attributes.class === "DataMap"
          ) {
            render = (
              <Container sx={{ my: 1 }} maxWidth="md">
        ...
        case "embed":
          //TODO
          const cdata = jsonElement.elements
            .filter((el) => (el.type = "CDATA"))
            .map((el) => el.cdata);
          ...
          if (jsonElement.attributes["emk-class"] === "card") {
            ...
            try {
              extraHeadline = jsonElement.elements.find(
                (subel) =>
                  subel.attributes["emk-type"] === "extra-content-headline"
              );
              ...
              <React.Fragment>
                {jsonElement.elements
                  ? jsonElement.elements
                      .filter(
                        (subel) =>
                          subel.attributes["emk-type"] !==
                          "extra-content-headline"
                      )
                      .map((subel, i) => (
                        <RenderContentElement
                          key={i}
...
function Figure({ jsonElement, excludeElements, neonData, renderMode }) {
  ...
  try {
    imageUrl = ResourceResolver(
      getImageFormatUrl(
        getImageUrl(jsonElement, "landscape", neonData),
        "large"
      ),
...
function ExtraLinks({ jsonElement, excludeElements, renderMode, neonData }) {
  ...
  try {
    headlineBlock = jsonElement.elements.find(
      (el) => el.attributes["emk-type"] === "extra-links-headline"
    );
    if (headlineBlock) {
      headlineBlock = (
        <React.Fragment>
  ...
  let linksBlock = null;
  try {
    linksBlock = jsonElement.elements.filter(
      (el) => el.attributes["emk-type"] === "extra-link"
    );
    if (linksBlock) {
      linksBlock = linksBlock.map((el, i) => {
        ...
        if (linkedObject) {
          ...
          try {
            ...
            linkedObjectMainImageUrl = getImageUrl(
              linkedObjectMainPictureElement,
              "square",
              neonData
            );
            if (linkedObjectMainImageUrl && linkedObjectMainImageUrl !== "#") {
              //TODO fix this
              linkedObjectMainImageUrl = getImageFormatUrl(
          ...
          blockRender = (
            <Box
              ...
              alignItems="center"
            >
              <Box sx={{ py: 1 }}>{linkImage ? linkImage : null}</Box>
  ...
  render = (
    <Container maxWidth="sm" sx={{ my: 4, border: 2, borderColor: "grey.500" }}>
      ...
      >
        {headlineBlock ? (
          <Box
            key="extra-links-headline"
            sx={{ borderBottom: 1, borderColor: "grey.500", my: 1 }}
          >
