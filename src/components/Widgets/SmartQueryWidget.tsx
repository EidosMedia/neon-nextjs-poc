 ...
 */
export default function SmartQueryWidget({ neonData, semanticSearchData, gridContext }) {
    let render = null;
    // let relevancyThreshold = 0.80
    // switch(neonData.linkContext.linkData.parameters.relevancy){
    //     case 'high':
    ...
    switch (neonData.linkContext.linkData.parameters.variety) {
        case 'high':
            varietyThreshold = 0.9;
            break;
        case 'medium':
    ...
    const boxTitle = neonData.linkContext.linkData.parameters.topic;
    ...
    if (list) {
        ...
        render = (
            <Container maxWidth="lg">
                {boxTitle ? (
                    <Box
                        sx={{ p: 1, mb: 2, backgroundColor: 'secondary.main' }}
                        display="flex"
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12} md={8}>
                        {list.slice(0, 1).map((object, i) => (
                            <SmartQueryFragment data={object} includeSummary={true} gridContext={{ xs: 12, md: 8 }} />
                        ))}
                    </Grid>
                    <Grid item xs={12} md={4}>
                        {list.slice(1, 3).map((object, i) => (
                            <SmartQueryFragment data={object} includeSummary={true} gridContext={{ xs: 12, md: 4 }} />
                        ))}
                    </Grid>
