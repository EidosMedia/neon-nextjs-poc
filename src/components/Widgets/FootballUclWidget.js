...
export default function FootballUclWidget({ cobaltData, gridContext }) {
    let render = null;
    if (cobaltData) {
        let match = null;
        let type = null;
        try {
            match = cobaltData.linkContext.linkData.parameters.match;
            type = cobaltData.linkContext.linkData.parameters.type;
        } catch (e) { }

        if (!match) {
            match = widgetsConfig.ucl.params.find((param) => param.name === 'match').defaultValue
        }
        if (!type) {
            type = widgetsConfig.ucl.params.find((param) => param.name === 'type').defaultValue
        }
            ...
            render = (
                <Container sx={{ my: 2, p:2, backgroundColor: 'grey.300'}} maxWidth="lg">
                    <Box sx={{display: 'flex', justifyContent:'center', alignItems:'center'}}>
                        ...
                        <Typography sx={{mx:3}} variant="h4">{teamAName}</Typography>
                        <Typography sx={{mx:3}} variant="h2">{score}</Typography>
                        <Typography sx={{mx:3}} variant="h4">{teamBName}</Typography>
