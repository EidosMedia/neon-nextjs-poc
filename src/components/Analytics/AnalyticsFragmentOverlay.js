...
export default function AnalyticsFragmentOverlay({
  neonData,
  analyticsReport,
  children,
}) {
  ...
  try {
    views =
      analyticsReport.realtimeReport.rows.find(findFn).metricValues[0].value;
  } catch (e) {}
  ...
  const hasGaReport = analyticsReport.contentReport.some(
    (r) => r.neonData.object.data.id === neonData.object.data.id
  );
    ...
    render = (
      <Dialog onClose={handleClose} open={open} fullWidth={true} maxWidth="md">
        ...
        >
          ...
          <RealtimeSummary
            neonData={props.neonData}
            realtimeReport={props.analyticsReport.realtimeReport}
          />
        </Box>
        <Box
          sx={{
