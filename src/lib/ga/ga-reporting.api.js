  ...
  if (data) {
    ...
    if (COMMON_GA_CACHE_ENABLED) {
      cacheData.put(
        cacheKey,
        response,
        COMMON_GA_CONTENT_CACHE_TTL_SECONDS * 1000
      );
    }
    return response;
  }
  ...
  if (data) {
    ...
    if (COMMON_GA_CACHE_ENABLED) {
      cacheData.put(
        cacheKey,
        response,
        COMMON_GA_REALTIME_CACHE_TTL_SECONDS * 1000
      );
    }
    return response;
  }
...
async function getSegmentAnalyticsReport(neonData) {
  ...
  const reports = await getGaMultiContentReport(
    objects.map((object) => object.object.data.id)
  );
...
async function getPageAnalyticsReport(neonData) {
  ...
  if (url) {
    url = url + neonData.pageContext.url;
  }
