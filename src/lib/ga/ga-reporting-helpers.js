        ...
        dimension2Values.forEach((dimension2Value, hi) => {
            ...
            if (reportsForDimensions1and2) {
                const dimension1and2AggregateValue = reportsForDimensions1and2.reduce(
                    (acc, report) => acc + parseInt(report.metricValues[metricIndex].value),
                    0
                );
 ...
 * @param neonData
 * @param contentReport
 * @param topContentPagesReport
 * @param dimension1
 * @param metric
 */
export function getTableDataFromContentReport(neonData, contentReport, topContentPagesReport, dimension1, metric) {
    // getting the index of the dimensions & metrics columns
    const contentDimension1Index = contentReport.dimensionHeaders.findIndex(header => header.name === dimension1);
    const topPagesDimension1Index = topContentPagesReport.dimensionHeaders.findIndex(
    ...
    const topPagesPagePathIndex = topContentPagesReport.dimensionHeaders.findIndex(
    ...
    const rowData = dimension1Values.reduce((acc, dimension1Value, i) => {
        const contentRowsForDimension1 = contentReport.rows.filter(
            row => row.dimensionValues[contentDimension1Index].value === dimension1Value
        );
        const topPageRowsForDimension1 = topContentPagesReport.rows.filter(
            row => row.dimensionValues[topPagesDimension1Index].value === dimension1Value
        );
        ...
        if (contentRowsForDimension1) {
            row[1] = contentRowsForDimension1.reduce(
                (acc, report) => acc + parseInt(report.metricValues[contentMetricIndex].value),
                0
            );
        } else {
            row[1] = 0;
        ...
        if (topPageRowsForDimension1) {
            const rank = topPageRowsForDimension1.findIndex(row =>
                row.dimensionValues[topPagesPagePathIndex].value.includes(neonData.object.data.id)
            );
            row[2] = rank > 0 ? rank : 0;
        } else {
            row[2] = 0;
