    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            for (let j = 0; j < 2; j++) {
                ...
                let l2id = await createSection("/first" + i + "/second" + j, l1id, authToken)

                for (let k = 0; k < 100; k++) {
                    let l3id = await createSection("/first" + i + "/second" + j + "/third" + k, l2id, authToken)
                }
            }
...
async function createSection(section, parentId, authToken) {
    ...
    let options = {
        method: 'POST',
        url: process.env.COBALT_BASE_HOST + '/core/sites/nodes/create?realm=default&emauth=' + authToken,
        mode: 'no-cors',
        data: body,
        headers: {
            'Accept': 'application/json',
    ...
    options = {
        method: 'POST',
        url: process.env.COBALT_BASE_HOST + '/core/sites/nodes/publish?realm=default&viewStatus=LIVE&emauth=' + authToken,
        mode: 'no-cors',
        data: body,
        headers: {
            'Accept': 'application/json',
